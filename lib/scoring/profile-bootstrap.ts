...
export async function ensureProfileScoringBootstrap(userId: string): Promise<void> {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("id, current_points, scoring_bootstrapped_at")
    .eq("id", userId)
    .maybeSingle();

  if (!profile) return;
  if (profile.scoring_bootstrapped_at) return;
  ...
  if (matchIds.length > 0) {
    const { data: matches } = await supabase
      .from("matches")
      .select("id, status, winner, bonus_result")
      .in("id", matchIds)
      .eq("status", "completed");
    ...
    const { data: existingMatchLedger } = await supabase
      ...
      .in("source_id", completedMatchIds.length > 0 ? completedMatchIds : ["00000000-0000-0000-0000-000000000000"]);
    ...
    const { data: prompts } = await supabase
      ...
      .eq("season_year", SEASON_YEAR)
      .eq("scope", "match")
      .in("match_id", completedMatchIds.length > 0 ? completedMatchIds : ["00000000-0000-0000-0000-000000000000"])
  ...
  const cur = Number(profile.current_points ?? 0);
  await supabase
    .from("profiles")
    .update({
