/**
 * Apply Round of 32 fixture corrections (M73–M88) from docs/fifa/matches.csv.
...
async function main() {
  ...
  const { data: matchRows, error: matchErr } = await supabase
    ...
    .gte("match_number", R32_FIRST)
    .lte("match_number", R32_LAST);
  ...
  const r32Rows = (matchRows ?? []).filter(
    (r) =>
      typeof r.match_number === "number" &&
      r.match_number >= R32_FIRST &&
      r.match_number <= R32_LAST,
  );

  if (!r32Rows.length) {
    console.error(`No matches with match_number ${R32_FIRST}–${R32_LAST} found.`);
    process.exit(1);
  }
  ...
  const allAliasIds = [...new Set(r32Rows.map((r) => r.id as string))];
  ...
  if (blockedFixtureNumbers.size) {
    ...
    for (const n of blocked) {
      const row = r32Rows.find((r) => r.match_number === n);
      if (row) {
        console.log(
  ...
  const onlyR32 = Array.from({ length: R32_LAST - R32_FIRST + 1 }, (_, i) => R32_FIRST + i);
  const result = await importFifaMatches(supabase, fifaDir, 2026, {
    onlyMatchNumbers: onlyR32,
  ...
  if (!allowed.length) {
    console.log("No fixtures updated — all R32 matches have predictions.");
    return;
  }
  ...
  console.log(`\nUpdated ${allowed.length} R32 fixture(s) from CSV (upserted ${result.upserted} total rows):`);
